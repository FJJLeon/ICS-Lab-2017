...
typedef enum {STAT_AOK, STAT_HLT, STAT_ADR, STAT_INS} stat_t;//CS:APP P364
...
bool_t get_byte_val(mem_t *m, long_t addr, byte_t *dest)//read one byte form memory
{
...
bool_t get_long_val(mem_t *m, long_t addr, long_t *dest)//read 8 byte from memory
{
    int i;
    long_t val;
...
long_t compute_alu(alu_t op, long_t argA, long_t argB)
{
    ...
    switch(op){
		...
		case A_SUB: val = argB - argA; break;//sub rA rB,is rB -rA
...
{//CA:APP P357
    ...
    switch (icode) {
      	...
      	case I_CALL: /* 8:x imm */
		{
			...
			long_t valid;
			if (!get_long_val(sim->m, rsp, &valid)){

...
#define READ(p) 		((unsigned int *)(long)(GET(p))) 
...
static void delete(void *bp, size_t asize);
...
static void *seg_first_fit(size_t asize){
	...
	for (index = get_index(asize); index <=9; index++){
		...
		char *bp =READ(seg_head + index * WSIZE); // the first list node or null
		while (bp != NULL) {
			printf("get size %d\n",GET_SIZE(HDRP(bp)));    
			if (GET_SIZE(HDRP(bp)) >= asize)
				return bp;
			bp = READ(NEXT_NODE(bp));
		}
	}
...
static void delete(void *bp, size_t asize) {
	...
	char *root = seg_head + index*WSIZE;
	char *pred = READ(PREV_NODE(bp));
	char *next = READ(NEXT_NODE(bp));

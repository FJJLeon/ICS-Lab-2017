...
//GET_SIZE(HDRP(((long)mem_heap_lo() & 0xffff00000000) | (long)READ(root))))
...
/*
static long *READ(unsigned int *bp){
...
static void *coalesce(void *bp)
{
	...
	//printf("coalesce %d\n", size);
	//printf("after coalsce bp size %d\n", GET_SIZE(HDRP(bp)));
	return bp;
}
...
static void *seg_first_fit(size_t asize){
	...
	for (index = get_index(asize); index <=9; index++){
		...
		//printf("seg fit asize: %d index: %d\n",asize, index);    
		
		char *bp = heap_listp + (long)READ(seg_head + index * WSIZE); // the first list node or null
		//printf("is heaplistp: %d\n", bp == heap_listp);

		while (bp != heap_listp) {
			//printf("get size %d at offset:%d\n",GET_SIZE(HDRP(bp)), (long)(bp - heap_listp));    
...
static void place(void *bp, size_t asize)
{
	...
	//printf("place 1 asize:%d,csize:%d\n",asize,csize);
	delete(bp);
...
//printf("insert size: %d index: %d, read(root) is %d\n", asize, index, READ(root)); //9
	/*
	while (next != NULL) {
...
printf("insert bp size is %d\n", GET_SIZE(HDRP(GET(root))));
	...
	*/
	...
	if (next == heap_listp) { //because use offset if null should be head_listp!!!!!!
		...
		//printf("insert %d, root point offset %d\n", GET_SIZE(HDRP(bp)),(long)READ(root));
		//printf("insert success?? %d\n",GET_SIZE(HDRP(heap_listp + (long)READ(root))) );
	}
	else {
		...
		//printf("heap first %p, last %p \n", mem_heap_lo(), mem_heap_hi());
		//printf("insert %d, root point offset %d\n",GET_SIZE(HDRP(bp)),(long)READ(root));
		//char *tmp = heap_listp + (long)READ(root);
		//printf("insert success?? %d\n",GET_SIZE(HDRP(tmp)));
		//tmp = heap_listp + (long)READ(NEXT_NODE(bp));
		//printf("second onde is %d\n", GET_SIZE(HDRP(tmp)));
	}
	return;
}
...
static void delete(char *bp) {
	...
	if (pred == heap_listp && next == heap_listp) {
		...
		WRITE(NEXT_NODE(bp), NULL);
		//printf("next size : %d offset %d\n",GET_SIZE(HDRP(next)), (long)READ(root));
	}
	else if (pred != heap_listp && next == heap_listp) {
		//printf("delete case 3\n");
		//printf("before delete bp offset %d \n",(long)(bp - heap_listp));
		// bp is the last
		WRITE(NEXT_NODE(pred), NULL);
		WRITE(PREV_NODE(bp), NULL);
		//printf("after delete prev node point at %d,  %d\n",(long)READ(NEXT_NODE(pred)),(long)READ(NEXT_NODE(bp)));
	}
	else {
		//printf("delete case 4\n");
		// within 
		//printf("NEXT_NODE offset is:%d and PREV %d\n",(long)READ(NEXT_NODE(bp)), (long)READ(PREV_NODE(bp)) );

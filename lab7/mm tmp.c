...
static void *seg_first_fit(size_t asize){
	...
	for (index = get_index(asize); index <=9; index++){
		...
		char *bp =heap_listp + READ(seg_head + index * WSIZE); // the first list node or null
		while (bp != NULL) {
			printf("get size %d\n",GET_SIZE(HDRP(bp)));    
			if (GET_SIZE(HDRP(bp)) >= asize)
				return bp;
			bp = heap_listp + READ(NEXT_NODE(bp));
		}
	}
...
printf("insert bp size is %d\n", GET_SIZE(HDRP(GET(root))));
	...
	*/
	...
	if (next == NULL) {
		...
		printf("insert %d, bp at %p root point at %p\n", GET_SIZE(HDRP(bp)), bp, heap_listp + READ(root));
		printf("gg...%d\n",GET_SIZE(HDRP(heap_listp + READ(root))) );
	}
	else {
		// this list is not empty, insert bp next to root  
		printf("list not empty\n");
		
		WRITE(NEXT_NODE(bp), heap_listp + READ(root));
		WRITE(PREV_NODE(heap_listp + READ(root)), bp - heap_listp);
		WRITE(PREV_NODE(bp), NULL);
		WRITE(root, bp - heap_listp);

		printf("heap first %p, last %p \n", mem_heap_lo(), mem_heap_hi());
		printf("insert %d, bp at %p root point at %p\n", GET_SIZE(HDRP(bp)), bp, heap_listp + READ(root));
		printf("gg...%d\n",GET_SIZE(HDRP(((long)(heap_listp + READ(root))))));
	}
	return;
}
...
static void delete(void *bp) {
	...
	char *root = seg_head + index*WSIZE;
	char *pred = heap_listp + READ(PREV_NODE(bp));
	char *next = heap_listp + READ(NEXT_NODE(bp));
	...
	if (pred == NULL && next == NULL) {
		...
		WRITE(root, heap_listp + READ(NEXT_NODE(bp)));
		WRITE(NEXT_NODE(bp), NULL);
	}
	else if (pred != NULL && next == NULL) {
		// bp is the last
		...
		// within 
		WRITE(NEXT_NODE(pred), heap_listp + next);
		WRITE(PREV_NODE(next), heap_listp + pred);
	}
